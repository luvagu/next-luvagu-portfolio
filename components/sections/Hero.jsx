...
function Hero() {
    ...
    const item1 = (<h1 className="mt-0 mr-0 mb-6 ml-1 text-yellow-400 font-mono text-base sm:text-lg font-normal leading-none">
        Hello, my name is
    </h1>)

    const item2 = (<h2 className="m-0 text-gray-300 text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-semibold leading-none">
        Luis Vallejo.
    </h2>)

    const item3 = (<h3 className="mt-3 text-gray-500 text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-semibold leading-none">
    ...
    return (
        <section className="flex justify-center items-start flex-col min-h-screen p-0 my-0 mx-auto max-w-5xl">
            <TransitionGroup component={null}>
                {isMonted && heroItems.map((item, idx) => (
                    <CSSTransition key={idx} classNames="fadeup" timeout={loaderDelay}>
                        <div style={{ transitionDelay: `${(idx + 1) * 100}ms` }}>{item}</div>
                    </CSSTransition>
                ))}
            </TransitionGroup>
